...
fq.all <- qf(pv, df.all, df1)          # Original dataset still has same number of df.
...
eofplts <- lapply(seq_along(eofcols), function(i) {
    suppressWarnings(plot_field(eofvis[i,], griddata, 14, -0.9, 0.9))
})
...
There is a lot to chew on here.  EOF-4 looks plausibly related to the PDO, and
EOF-3 seems consistent with ENSO.  We don't expect any of these modes to
correspond _exactly_ with the classic oscillations, both because we're learning
the behavior from a relatively small amount of data and because many of those
phenomena are observed in sea surface temperature, and we're looking at
near-surface air temperature.

EOF-25 and EOF-50 are also interesting.  They show progressively smaller spatial
scale for fluctuations.  Since the power also decreases as we go further down
the list, we can conclude that most of the power in the random fluctuations is
at longer spatial scales, particularly those associated with periodic phenomena.
In fact, the majority of the power is in those first 10 modes:
```{r powerfrac}
sum(eofpwr$relpwr[eofpwr$EOF < 10]) / sum(eofpwr$relpwr)
```
