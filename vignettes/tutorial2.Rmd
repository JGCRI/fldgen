---
title: "Fldgen v2 Tutorial - Joint Temperature and Precipitation"
...
```
infileT <- file.path(datadir, 'tas_annual_esm_rcp_r2i1p1_startyr-endyr.nc')
infileP <- file.path(datadir, 'pr_annual_esm_rcp_r2i1p1_startyr-endyr.nc')
emulator <- trainaTP(c(infileT, infileP), Ngrid = 55296,
...
The ESM temperature and precipitation fields should be in netCDF filea.  The function that reads it returns a "griddata" structure that contains the data, as well as some information about the grid, such as the latitude, longitude, and time coordinates and the vector of area weights needed to compute grid averages.

```{r readdata}
filenameT <- file.path(datadir, 'tas_annual_esm_rcp_r2i1p1_startyr-endyr.nc')
...
filenameP <- file.path(datadir, 'pr_annual_esm_rcp_r2i1p1_startyr-endyr.nc')
...
Now we're ready to generate fields.  We'll do `r ngen` fields in this example. The first one will be a reconstruction of the original input.  The other three will be new fields. Because the EOF analysis was performed on a joint matrix of _transformed_ T and P residuals,  each grid cell in a generated field follows $N(0,1)$.
...
Finally, the mean field is reconstructed and added to each residual field to create new, full global gridded temperature and precipitation realizations. 
...
fullgrids <- lapply(residgrids, function(g){
    g[, 1:Ngrid] <- g[, 1:Ngrid] + meanfieldT
    g[, (Ngrid+1):(2*Ngrid)] <- g[, (Ngrid+1):(2*Ngrid)] + meanfieldP
...
plotglobalfieldsP <- function(fieldlist, sequencenum, minval=-5e-5, maxval=5e-5, legendstr) {
    ## Extract a single example field from each series (sequencenum determines which one) and create a plot
    lapply(fieldlist, function(g) {
        suppressWarnings(
...
residfieldplotsP <- plotglobalfieldsP(residgrids[1:nplt], exfld, legendstr='delta-P (flux)')
