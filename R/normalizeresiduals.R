...
normalize.resids <- function(r, len=NULL){
    ...
    # Get the quantiles of every residual for the grid cell
    ecdf(r)(r) -> quantiles
    ...
    (1-max2) / 5 -> offset
    quantiles[which.max(quantiles)] <- max(quantiles) - offset


    # get the values of the normal distribution corresponding to these quantiles.
    normresids <- qnorm(quantiles, mean = 0, sd = 1)


    # return
    list(r = r, quants = quantiles, rn = normresids)
